...
def mock_supabase():
    ...
    # The execute function itself needs to be a MagicMock (sync) returning the result object
    execute_mock = MagicMock(return_value=result_obj)
    ...
    # Also attach the execute mock to client just in case (though table() usually starts it)
    # But importantly, client.table -> query_builder.
    
    return client

@pytest.fixture
def mock_config(mock_supabase):
    """Mocks the supabase client in database module."""
    with pytest.MonkeyPatch.context() as m:
        # Patch the alias in database module because it does 'from config import supabase'
        # If we only patch config.supabase, database.supabase remains the original object
